...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (temp > 8.0) {
      if (!advisoryGeneratedRef.current) {
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className={`rounded-2xl border flex flex-col shadow-lg shadow-black/20 overflow-hidden ${isWarning ? 'bg-red-950/20 border-red-500/30' : 'bg-slate-800 border-slate-700'}`}>
          ...
          {isWarning && <div className="p-3 bg-red-500/10 border-t border-red-500/20 text-[10px] text-red-300 flex items-center gap-2"><Info size={14} />Advisory generated by Gemini 1.5 Pro.</div>}
        </div>
      </div>
    </div>
  );
