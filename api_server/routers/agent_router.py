    ...
    },
    {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "alert_type": {
                    "type": "string",
                    "enum": ["temperature", "battery", "door_open", "route", "delay"],
...
async def run_agent(db: Session = Depends(get_db)):
    ...
    if not api_key or api_key == "YOUR_ACTUAL_GEMINI_API_KEY_HERE" or api_key == "put_your_real_gemini_key_here":
        raise HTTPException(
            status_code=503,
            detail="VITE_GEMINI_API_KEY is not set or is still the default placeholder. Cannot run agent."
        )
...
CRITICAL INSTRUCTION: Do NOT output your final report in JSON format. Your final report MUST be written in beautiful, highly readable Markdown format with proper headings (##), bullet points, and bold text for emphasis. Be decisive. Create alerts proactively. Your job is to catch issues before they cause vaccine spoilage."""
            ...
            # ReAct loop — keep going while the agent wants to call tools
            while True:
                step += 1
                if step > 15:  # Safety limit
                    break

                # Check if agent is calling a function
                fn_calls = []
                ...
                # Send all tool results back to the agent
